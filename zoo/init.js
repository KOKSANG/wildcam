...
var randlocation = [{
    name: "Terengganu",
    loc: [103.205299, 5.207053, 10]
  },
  {
    name: "Kelantan",
    loc: [102.242203, 6.139872, 10]
  },
  {
    name: "Kedah",
    loc: [100.569649, 6.155672, 10]
  },
  {
    name: "Selangor",
    loc: [101.538116, 3.519863, 10]
  },
  {
    name: "Johor",
    loc: [103.358727, 1.934400, 10]
  }
]

var ripple = [];

function add_ripple(label) {
  document.getElementById('notify').innerHTML = '';
  document.getElementById('notify').innerHTML = label + ' IS DETECTED!'; 
  var randn = Math.floor(Math.random() * 4);
  var animal_label = label;
  console.log(label)
  ripple.push({
    animal: animal_label,
    name: randlocation[randn].name,
    value: randlocation[randn].loc
  });
...
var socket = io.connect('http://huseinhouse.com:8020/live');
socket.on('camera_update', function (message) {
  add_ripple(message.label);
});
